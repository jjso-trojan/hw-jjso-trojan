...
public:
  ...
  // Then it should insert the token at index%n where n is the new size of the
  // list and assuming that the head of the list is at index 0.
  ...
  // Otherwise, remove should remove the item at index%n where n is the current
  // size of the list assuming that the head of the list is at index 0.
  ...
  friend std::ostream &operator<<(std::ostream &out, TokenList &List);
