...
FlexibleCharacterManager::FlexibleCharacterManager()
    : blocks(new CharacterBlock[2]{}) {}
...
char *FlexibleCharacterManager::alloc_chars(size_t n) {
  ...
  size_t emptySlotNum =
      0;               // index of the free block that is bigger or equal 'n'
  ...
  while (emptySlotNum < blocks_in_use) {
    if (blocks[emptySlotNum].address[0] == '\0' &&
        n <= blocks[emptySlotNum].size) {
  ...
  if (emptySlotNum == blocks_in_use) {
    ...
    blocks[emptySlotNum + 1] = {blocks[emptySlotNum].address +
                                    blocks[emptySlotNum].size,
                                newEmptyBlockSize};
    blocks_in_use++;
  }
  return addr;
...
void FlexibleCharacterManager::free_chars(char *p) {
  ...
  // step #3. reduce 'blocks_size' half if blocks_in_use is less than half of
  // it.
